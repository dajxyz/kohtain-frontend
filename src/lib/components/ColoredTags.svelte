<script>
    ...
    export let addKeys = [13]; // Default is Enter key
    export let maxTags = false;
    export let onlyUnique = false;
    export let removeKeys = [8]; // Default is Backspace
    ...
    export let autoCompleteStartFocused = false;
    ...
    export let onTagAdded = function(){};
    export let onTagRemoved = function(){};
    export let cleanOnBlur = false;
    export let customValidation = false;
    
    // Track tag colors mapping
    let tagColors = {};
    let autoCompleteIndexStart = autoCompleteStartFocused ? 0 : -1;
    let autoCompleteIndex = autoCompleteIndexStart;
    let matchsID = id + "_matchs";
    let layoutElement;
    let storePlaceholder = placeholder;
    
    // Function to assign color to a tag
    function getTagColor(tag) {
      if (typeof tag === 'object' && tag !== null && autoCompleteKey) {
        const key = tag[autoCompleteKey];
        
        // If color is already assigned to this tag, return it
        if (tagColors[key]) return tagColors[key];
        
        // Assign a new color from the array
        const colorIndex = Object.keys(tagColors).length % highlightColours.length;
        tagColors[key] = highlightColours[colorIndex];
        ...
        // Assign a new color from the array
        const colorIndex = Object.keys(tagColors).length % highlightColours.length;
        tagColors[tag] = highlightColours[colorIndex];
        return tagColors[tag];
      }
      
      // Default color if something goes wrong
      return highlightColours[0];
    }
      ...
      if (removeKeys) {
        removeKeys.forEach(function(key) {
          if (key === e.keyCode && tag === "") {
            ...
            // Remove the color mapping if the tag is removed
            if (typeof removedTag === 'object' && removedTag !== null && autoCompleteKey) {
              delete tagColors[removedTag[autoCompleteKey]];
            } else if (typeof removedTag === 'string') {
    ...
    function addTag(currentTag) {
      ...
      const tagToAdd = currentObjTags ? currentObjTags : currentTag;
      
      // Ensure the tag has a color assigned before adding it
      getTagColor(tagToAdd);
      ...
      {#if tags.length > 0}
        {#each tags as tag, i}
          <button 
            ...
            style="background-color: {typeof tag === 'string' ? getTagColor(tag) : getTagColor(tag)};"
          >
