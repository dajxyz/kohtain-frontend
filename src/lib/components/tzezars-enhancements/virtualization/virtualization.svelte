...
<script lang="ts" generics="ItemType">
	...
	function updateVisibleItemsInfo() {
		if (!items || isDisabled) {
			return [];
		}
		const r = [];
	...
	// 	propsUpdated();
	// 	visibleItemsInfo = updateVisibleItemsInfo();
	// });
	$effect(() => {
...
<div
	...
	style={listStyle}
>
	<div bind:this={listInner} class="!w-full min-w-max" style={listInnerStyle}>
		{@render header?.()}
